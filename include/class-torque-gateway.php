<?php
/*
 * Main Gateway of Torque using either a local daemon or the explorer
 * Authors: Serhack, cryptochangements, mosu-forge
 */

defined( 'ABSPATH' ) || exit;

require_once('class-torque-cryptonote.php');

class Torque_Gateway extends WC_Payment_Gateway
{
    private static $_id = 'torque_gateway';
    private static $_title = 'Torque Gateway';
    private static $_method_title = 'Torque Gateway';
    private static $_method_description = 'Torque Gateway Plug-in for WooCommerce.';
    ...
    private static $use_torque_price = false;
    private static $use_torque_price_decimals = TORQUE_GATEWAY_ATOMIC_UNITS;

    private static $cryptonote;
    private static $torque_wallet_rpc;
    private static $torque_explorer_tools;
    private static $log;

    private static $currencies = array('price_btc','price_usd');
    ...
    {
        return apply_filters('woocommerce_gateway_icon', '<img src="'.TORQUE_GATEWAY_PLUGIN_URL.'assets/images/torque-icon.png"/>', $this->id);
    }

    function __construct($add_action=true)
    {
        $this->id = self::$_id;
        $this->method_title = __(self::$_method_title, 'torque_gateway');
        $this->method_description = __(self::$_method_description, 'torque_gateway');
        ...
        self::$address = $this->settings['torque_address'];
        ...
        self::$use_torque_price = $this->settings['use_torque_price'] == 'yes';
        self::$use_torque_price_decimals = $this->settings['use_torque_price_decimals'];

        $explorer_url = self::$testnet ? TORQUE_GATEWAY_TESTNET_EXPLORER_URL : TORQUE_GATEWAY_MAINNET_EXPLORER_URL;
        defined('TORQUE_GATEWAY_EXPLORER_URL') || define('TORQUE_GATEWAY_EXPLORER_URL', $explorer_url);

        if($add_action)
            add_action('woocommerce_update_options_payment_gateways_'.$this->id, array($this, 'process_admin_options'));

        // Initialize helper classes
        self::$cryptonote = new Torque_Cryptonote();
        if(self::$confirm_type == 'torque-wallet-rpc') {
            require_once('class-torque-wallet-rpc.php');
            self::$torque_wallet_rpc = new Torque_Wallet_Rpc(self::$host, self::$port);
        } else {
            require_once('class-torque-explorer-tools.php');
            self::$torque_explorer_tools = new Torque_Explorer_Tools(self::$testnet);
        }
    ...
    {
        $this->form_fields = include 'admin/torque-gateway-admin-settings.php';
    }

    public function validate_torque_address_field($key,$address)
    {
        if($this->settings['confirm_type'] == 'viewkey') {
            if (strlen($address) == 95 && substr($address, 0, 1) == '4')
                if(self::$cryptonote->verify_checksum($address))
                    return $address;
            self::$_errors[] = 'Torque address is invalid';
        }
        return $address;
    }
    ...
    public function admin_options()
    {
        ...
        if($confirm_type === 'torque-wallet-rpc')
        ...
        include TORQUE_GATEWAY_PLUGIN_DIR . '/templates/torque-gateway/admin/settings-page.php';
    }
    ...
    public static function admin_balance_info()
    {
        ...
        $wallet_amount = self::$torque_wallet_rpc->getbalance();
        $height = self::$torque_wallet_rpc->getheight();
        if (!isset($wallet_amount)) {
            self::$_errors[] = 'Cannot connect to torque-wallet-rpc';
            self::$log->add('Torque_Payments', '[ERROR] Cannot connect to torque-wallet-rpc');
            return array(
                'height' => 'Not Available',
                'balance' => 'Not Available',
                'unlocked_balance' => 'Not Available',
            ...
            return array(
                'height' => $height,
                'balance' => self::format_torque($wallet_amount['balance']).' Torque',
                'unlocked_balance' => self::format_torque($wallet_amount['unlocked_balance']).' Torque'
            );
        }
    }
    ...
    public function process_payment($order_id)
    {
        ...
        if(self::$confirm_type != 'torque-wallet-rpc') {
          ...
          // Generate subaddress
          $payment_id = self::$torque_wallet_rpc->create_address(0, 'Order: ' . $order_id);
          if(isset($payment_id['address'])) {
            $payment_id = $payment_id['address'];
          }
          else {
            $this->log->add('Torque_Gateway', 'Couldn\'t create subaddress for order ' . $order_id);
          }
        }
        ...
        $torque_amount = 1e8 * $fiat_amount / $rate;

        if(self::$discount)
            $torque_amount = $torque_amount - $torque_amount * self::$discount / 100;

        $torque_amount = intval($torque_amount * TORQUE_GATEWAY_ATOMIC_UNITS_POW);

        $query = $wpdb->prepare("INSERT INTO $table_name (order_id, payment_id, currency, rate, amount) VALUES (%d, %s, %s, %d, %d)", array($order_id, $payment_id, $currency, $rate, $torque_amount));
        $wpdb->query($query);

        $order->update_status('on-hold', __('Awaiting offline payment', 'torque_gateway'));
    ...
    public static function do_update_event()
    {
        ...
        $api_link = 'https://api.coinmarketcap.com/v1/ticker/torque/';
        $curl = curl_init();
        ...
        if(self::$confirm_type == 'torque-wallet-rpc')
            ...
            $height = self::$torque_explorer_tools->getheight();
        set_transient('torque_gateway_network_height', $height);

        // Get pending payments
        $table_name_1 = $wpdb->prefix.'torque_gateway_quotes';
        $table_name_2 = $wpdb->prefix.'torque_gateway_quotes_txids';
        ...
        foreach($pending_payments as $pending) {
            ...
            $amount_torque = $quote->amount_total;
            ...
            $paid = $amount_paid > $amount_torque - TORQUE_GATEWAY_ATOMIC_UNIT_THRESHOLD;
            ...
            if($paid && $confirmed) {
                self::$log->add('Torque_Payments', "[SUCCESS] Payment has been confirmed for order id $order_id and payment id $payment_id");
                ...
                if(self::is_virtual_in_cart($order_id) == true){
                    $order->update_status('completed', __('Payment has been received.', 'torque_gateway'));
                } else {
                    $order->update_status('processing', __('Payment has been received.', 'torque_gateway'));
                }

            } else if($paid) {
                self::$log->add('Torque_Payments', "[SUCCESS] Payment has been received for order id $order_id and payment id $payment_id");
                ...
                if($order_age_seconds > self::$valid_time) {
                    self::$log->add('Torque_Payments', "[FAILED] Payment has expired for order id $order_id and payment id $payment_id");
                    $query = $wpdb->prepare("UPDATE $table_name_1 SET pending=0 WHERE payment_id=%s", array($payment_id));
                    $wpdb->query($query);

                    unset(self::$payment_details[$order_id]);

                    $order->update_status('cancelled', __('Payment has expired.', 'torque_gateway'));
                }
            }
        }
    }
    ...
    {
        $txs = array();
        $address_index = self::$torque_wallet_rpc->get_address_index($subaddress);
        if(isset($address_index['index']['minor'])){
          $address_index = $address_index['index']['minor'];
        }
        else {
          self::$log->add('Torque_Gateway', '[ERROR] Couldn\'t get address index of subaddress: ' . $subaddress);
          return $txs;
        }
        $payments = self::$torque_wallet_rpc->get_transfers(array( 'in' => true, 'pool' => true, 'subaddr_indices' => array($address_index)));
    ...
    {
        $txs = array();
        $outputs = self::$torque_explorer_tools->get_outputs(self::$address, self::$viewkey);
        ...
        if (count($details)) {
            ...
            foreach($details as $tx) {
                ...
                $txs[] = array(
                    ...
                    'amount_formatted' => self::format_torque($tx->amount_paid)
                );
            ...
            if(count($heights) && !in_array(0, $heights)) {
                $height = get_transient('torque_gateway_network_height');
                $highest_block = max($heights);
            ...
            $time_to_confirm = self::format_seconds_to_time($blocks_to_confirm * TORQUE_GATEWAY_DIFFICULTY_TARGET);
            ...
            if(self::$confirm_type == 'torque-wallet-rpc') {
                ...
                if ($address) {
                    ...
                    self::$log->add('Torque_Gateway', '[ERROR] Merchant has not set Torque address');
                    return '[ERROR] Merchant has not set Torque address';
                }
            }
            ...
            $amount_formatted = self::format_torque($amount_due);
            $qrcode_uri = 'torque:'.$address.'?tx_amount='.$amount_formatted.'&tx_payment_id='.$payment_id;
            ...
            $payment_details = array(
                ...
                'amount_total_formatted' => self::format_torque($amount_total),
                'amount_paid_formatted' => self::format_torque($amount_paid),
                'amount_due_formatted' => self::format_torque($amount_due),
                'status' => $status,
    ...
    public static function get_payment_details_ajax() {
        ...
        if($order->get_payment_method() != self::$_id)
            self::ajax_output(array('error' => '[ERROR] Order not paid for with Torque'));
    ...
    public static function admin_order_page($post)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include TORQUE_GATEWAY_PLUGIN_DIR . '/templates/torque-gateway/admin/order-history-error-page.php';
            return;
        }
        include TORQUE_GATEWAY_PLUGIN_DIR . '/templates/torque-gateway/admin/order-history-page.php';
    }
    ...
    public static function customer_order_page($order)
    {
        ...
        if(!is_array($details)) {
            $error = $details;
            include TORQUE_GATEWAY_PLUGIN_DIR . '/templates/torque-gateway/customer/order-error-page.php';
            return;
        }
        $show_qr = self::$show_qr;
        $details_json = json_encode($details);
        $ajax_url = WC_AJAX::get_endpoint('torque_gateway_payment_details');
        include TORQUE_GATEWAY_PLUGIN_DIR . '/templates/torque-gateway/customer/order-page.php';
    }
    ...
    public static function customer_order_email($order)
    {
        ...
        $details = self::get_payment_details($order_id);
        if(!is_array($details)) {
            include TORQUE_GATEWAY_PLUGIN_DIR . '/templates/torque-gateway/customer/order-email-error-block.php';
            return;
        }
        include TORQUE_GATEWAY_PLUGIN_DIR . '/templates/torque-gateway/customer/order-email-block.php';
    }
    ...
    public static function use_torque_price()
    {
        return self::$use_torque_price;
    }
    ...
    {
        $rate = self::get_live_rate($currency);
        $torque_amount = intval(TORQUE_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / TORQUE_GATEWAY_ATOMIC_UNITS_POW;
        $torque_amount_formatted = sprintf('%.'.self::$use_torque_price_decimals.'f', $torque_amount);

        return <<<HTML
            <span class="woocommerce-Price-amount amount" data-price="$price" data-currency="$currency"
        data-rate="$rate" data-rate-type="live">
            $torque_amount_formatted
            <span class="woocommerce-Price-currencySymbol">XTC</span>
        </span>
...
HTML;
    ...
    public static function convert_wc_price_order($price_html, $order)
    {
        ...
        $torque_amount = intval(TORQUE_GATEWAY_ATOMIC_UNITS_POW * 1e8 * $price / $rate) / TORQUE_GATEWAY_ATOMIC_UNITS_POW;
        $torque_amount_formatted = sprintf('%.'.TORQUE_GATEWAY_ATOMIC_UNITS.'f', $torque_amount);
...
HTML;
    ...
    public static function format_torque($atomic_units) {
        return sprintf(TORQUE_GATEWAY_ATOMIC_UNITS_SPRINTF, $atomic_units / TORQUE_GATEWAY_ATOMIC_UNITS_POW);
    }
