 ...
 * Admin interface for Torque gateway
 * Authors: mosu-forge
 */
...
class Torque_Admin_Payments_List extends WP_List_Table {
    ...
    function extra_tablenav($which) {
        if ($which == "top") {
            ...
            if(Torque_Gateway::get_confirm_type() == 'torque-wallet-rpc') {
                $balance = Torque_Gateway::admin_balance_info();
                $balance_info = <<<HTML
...
HTML;
            ...
            echo <<<HTML
            <div class="wrap">
                <h1 class="wp-heading-inline">Torque Payments</h1>
                $balance_info
                <hr class="wp-header-end">
                <ul class="subsubsub">
                    <li>
                        <a href="?page=torque_gateway_payments&type=all" {$tab_info['all']['active']}>
                            All <span class="count">({$tab_info['all']['count']})</span>
                        </a> |
                    </li>
                    <li style="display:none">
                        <a href="?page=torque_gateway_payments&type=pending" {$tab_info['pending']['active']}>
                            Pending <span class="count">({$tab_info['pending']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=torque_gateway_payments&type=paid" {$tab_info['paid']['active']}>
                            Received <span class="count">({$tab_info['paid']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=torque_gateway_payments&type=confirmed" {$tab_info['confirmed']['active']}>
                            Confirmed <span class="count">({$tab_info['confirmed']['count']})</span>
                        </a> |
                    </li>
                    <li>
                        <a href="?page=torque_gateway_payments&type=expired" {$tab_info['expired']['active']}>
                            Expired <span class="count">({$tab_info['expired']['count']})</span>
                        </a>
                    </li>
                </ul>
                <form id="torque-payments-filter" method="get" style="display:none">
                    <p class="search-box">
                ...
                <h2 class="screen-reader-text">Torque Payments List</h2>
                <style>
...
HTML;
    ...
    public function column_default($item, $column_name) {
        ...
        case 'col_payment_id':
            ...
            break;
        case 'col_txid':
            $url = TORQUE_GATEWAY_EXPLORER_URL.'/tx/'.$item->txid;
        ...
        case 'col_height':
            ...
            break;
        case 'col_amount':
            echo Torque_Gateway::format_torque($item->amount).' Torque';
            break;
        }
    }
    ...
    public function no_items() {
        esc_html_e('No Torque payments found', 'torque_gateway');
    }
